...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    ASSERT_TRUE(parser.Parse(SplitString("app --help")));
    // Проверка закоментирована намеренно. Ождиается, что результат вызова
    // функции будет приблизительно такой же, но не с точностью до символа
    ...
    //     "\n"
    //     "-i,  --input=<string>,  File path for input file [repeated, min args
    //     = 1]\n"
